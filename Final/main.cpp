...
bool bubble_shooter_collision(vector<Bubble> bubbles, Shooter shooter)
{
    ...
    for (unsigned int i = 0; i < bubbles.size(); i++)
    {
        ...
        if (dx_head * dx_head + dy_head * dy_head < max_dr_head * max_dr_head)
        {
            return true;
        }
...
int main()
{
    ...
    while (true)
    {
        ...
        while (true)
        {
            ...
            // Update Time counter.
            if (time_counter - int(time_counter) < time_wait) // means time_counter is integer (to a good approximation)
            {
                if (int(time_counter) / 10 == 0)
                    ...
                    time[time.length() - 6] = (time_counter / 10) + '0';
                time[time.length() - 5] = (int(time_counter) % 10) + '0';
                // % operator works only on integers, we know here time_count is integer, but still stored in double, hence this conversion
                TIME.setMessage(time);
                if (time_counter <= 10)
